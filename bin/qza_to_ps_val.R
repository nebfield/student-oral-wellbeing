...
tax <- tax %>%
  tidyr::separate(Kingdom, c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species"), sep = ";",  extra = "drop", fill = "right")
...
missing_smokers <- !is.na(phyloseq::sample_data(ps)$smoking)
ps_pruned <- phyloseq::prune_samples(missing_smokers, ps)
...
saveRDS(ps_pruned, file = "ps_second.rds")
